...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const moAlarm = document.querySelector('.mo-alarm');

  // MutationObserver 설정
  const observer = new MutationObserver((mutationsList) => {
    mutationsList.forEach((mutation) => {
      if (mutation.attributeName === 'aria-modal') {
        const isModalTrue = alertList.getAttribute('aria-modal') === 'true';
        if (isModalTrue) {
          moAlarm.classList.add('on');
        } else {
          moAlarm.classList.remove('on');
        }
      }
    });
  });

  // `aria-modal` 속성 변화를 감지
  observer.observe(alertList, { attributes: true });
});
